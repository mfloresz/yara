...
func registerImportRoutes(api *pbrouter.RouterGroup[*core.RequestEvent], s *Server) {
	...
	api.POST("/db/novels/import-from-zip", func(e *core.RequestEvent) error {
		...
		parsed, err := epubimport.ParseZipNovel(blob)
		if err != nil {
			switch {
			case errors.Is(err, epubimport.ErrInvalidZip):
				return e.BadRequestError("invalid zip file", err)
			case errors.Is(err, epubimport.ErrMissingMetadata):
				return e.BadRequestError("metadata.json is required in the zip", nil)
			case errors.Is(err, epubimport.ErrMissingOriginals):
				return e.BadRequestError("originals/ directory is required in the zip", nil)
			default:
				return e.InternalServerError("failed to read zip entry", err)
			}
		}
		chapters := make([]store.ImportedZipChapter, 0, len(parsed.Chapters))
	...
	api.POST("/db/novels/preview-from-url", func(e *core.RequestEvent) error {
		...
		s.importInfoCache.set(cacheKey, info)
	...
	api.POST("/db/novels/import-from-url", func(e *core.RequestEvent) error {
		...
		var err error
		if cachedInfo, cached := s.importInfoCache.pop(cacheKey); cached {
			info = cachedInfo
		} else {
			info, err = s.getNovelInfoWithFallback(e.Request.Context(), e.Auth.Id, body.URL)
			if err != nil {
		...
		chTitle := ch.Title
		if chTitle == "" {
			chTitle = fmt.Sprintf("Chapter %d", startCh)
		}
		...
		if len(remainingChapters) > 0 {
			job := buildDownloadJob(result.Novel.ID, body.URL, remainingChapters, startCh+1, sourceLang, targetLang, false)
			jobID, queueFull, err := s.createAndEnqueueDownloadJob(e.Auth.Id, job)
			if err != nil {
	...
	api.GET("/db/novels/{id}/update-preview", func(e *core.RequestEvent) error {
		...
		diff := diffNewChapters(info.Chapters, existingOrders, existingTitles, 0, 0)
		if err := s.Store.UpdateNovelCheckResult(novelID, time.Now().Format(time.RFC3339), diff.newAvailable); err != nil {
		...
		return e.JSON(http.StatusOK, map[string]any{
			...
			"totalChapters":   len(info.Chapters),
			"newChapters":     diff.newAvailable,
			"firstNewChapter": diff.firstNew,
			"lastNewChapter":  diff.lastNew,
		})
	})
	api.POST("/db/novels/{id}/update-from-url", func(e *core.RequestEvent) error {
		...
		if cached, found := s.previewCache.pop(cacheKey); found {
			chapters = cached
		} else {
			dl := s.DownloaderFactory(e.Auth.Id)
			info, err := dl.GetNovelInfo(e.Request.Context(), novel.URL)
		...
		diff := diffNewChapters(chapters, existingOrders, existingTitles, body.StartChapter, body.EndChapter)
		if len(diff.newChapters) == 0 {
			return e.JSON(http.StatusOK, map[string]any{"chaptersAdded": 0, "chapters": []map[string]any{}, "totalChapters": len(chapters), "code": codeNoNewChapters, "message": "No new chapters. The novel is already up to date."})
		}
		job := buildDownloadJob(novelID, novel.URL, diff.newChapters, diff.startOrder, novel.SourceLanguage, novel.TargetLanguage, false)
		downloadJobID, queueFull, err := s.createAndEnqueueDownloadJob(e.Auth.Id, job)
		if err != nil {
		...
		return e.JSON(http.StatusOK, map[string]any{
			...
			"pendingChapters": len(diff.newChapters),
			"downloadJobId":   downloadJobID,
			"message":         fmt.Sprintf("Download started. %d chapters are downloading in the background.", len(diff.newChapters)),
		})
	})
	api.POST("/db/novels/{id}/redownload-from-url", func(e *core.RequestEvent) error {
		...
		if !body.Confirm {
			...
			if cached, ok := s.previewCache.pop(cacheKey); ok {
				chapters = cached
		...
		if len(plan.chapters) == 0 {
			return e.JSON(http.StatusOK, map[string]any{
				"pendingChapters": 0,
				"code":            codeNoChaptersToRedownload,
				"message":         "No chapters matched for re-download. Check that the novel has chapters or that the range is valid.",
			})
		}
		if !body.Confirm && len(plan.mismatches) > 0 {
			// Cache the fresh list so the confirmed request does not re-scrape.
			s.previewCache.set(cacheKey, chapters)
			return e.JSON(http.StatusOK, map[string]any{
		...
		if len(activeJobs) > 0 {
			return e.Error(http.StatusConflict, jobInProgressMessage, map[string]any{"code": jobConflictCode{}})
		}

		job := buildDownloadJob(novelID, novel.URL, plan.chapters, plan.chapters[0].Order, novel.SourceLanguage, novel.TargetLanguage, true)
		downloadJobID, queueFull, err := s.createAndEnqueueDownloadJob(e.Auth.Id, job)
		if err != nil {
			return e.InternalServerError("failed to create download job", err)
		}
		...
		return e.JSON(http.StatusOK, map[string]any{
			"pendingChapters": len(plan.chapters),
			"downloadJobId":   downloadJobID,
			"message":         fmt.Sprintf("Re-download started. %d chapters will be updated in the background, keeping their translations.", len(plan.chapters)),
		})
	})
	...
	api.POST("/db/novels/batch-translate", func(e *core.RequestEvent) error {
		...
		skipped := make([]store.BatchSkipResult, 0)
		totalPending := 0
		for _, sel := range body.Selections {
			if _, err := s.Store.GetOwnedNovel(e.Auth.Id, sel.NovelID); err != nil {
				skipped = append(skipped, store.BatchSkipResult{NovelID: sel.NovelID, Reason: "novel_not_found"})
				continue
			}
			...
			if len(sel.ChapterIDs) > 0 {
				...
				if err != nil || len(pending) == 0 {
					skipped = append(skipped, store.BatchSkipResult{NovelID: sel.NovelID, Reason: "no_pending_chapters"})
					continue
				}
				chapterIDs = pending
			}
			...
			if err := s.Store.CreateJob(e.Auth.Id, job); err != nil {
				slog.Warn("batch translate: create job", "novel", sel.NovelID, "error", err)
				skipped = append(skipped, store.BatchSkipResult{NovelID: sel.NovelID, Reason: "job_creation_failed"})
				continue
			}
		...
		return e.JSON(http.StatusOK, store.BatchTranslateStartResponse{
			Jobs: jobs, TotalPending: totalPending, Skipped: skipped,
		})
	})
	api.POST("/db/novels/batch-check", func(e *core.RequestEvent) error {
		...
		for _, novelID := range body.NovelIDs {
			novel, err := s.Store.GetOwnedNovel(e.Auth.Id, novelID)
			if err != nil {
				results = append(results, checkResult{NovelID: novelID, Error: "novel_not_found"})
				continue
			}
			if novel.URL == "" {
				results = append(results, checkResult{NovelID: novelID, Error: "novel_has_no_url"})
				continue
			}
			...
			if err := s.Store.CreateJob(e.Auth.Id, job); err != nil {
				slog.Warn("batch check: create job", "novel", novelID, "error", err)
				results = append(results, checkResult{NovelID: novelID, Error: "job_creation_failed"})
				continue
			}
