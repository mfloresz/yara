...
func loadEnvFile(t *testing.T) {
	...
	if err != nil {
		t.Skipf("no .env file at repo root: %v", err)
	}
	defer f.Close()
...
func TestRefineReal(t *testing.T) {
	loadEnvFile(t)

	apiKey := os.Getenv("OPENCODEGO_API_KEY")
	if apiKey == "" || apiKey == "your-api-key-here" {
		t.Skip("OPENCODEGO_API_KEY not set in .env — skipping real refine test")
	}
...
The sun was beginning to set, painting the sky in shades of orange and purple. He pulled his coat tighter around his shoulders and stood up slowly, his joints protesting every movement. He would go home now, to the empty apartment, to the silence that greeted him at the door. Tomorrow he would come back to this bench, and the day after that, and the day after that. It was the only routine he had left.`
	...
	// Translation with intentional errors:
	// - "vacia" → missing accent on "vacía"
	// - "las observaban" → should be "los observaban" (niños is masculine)
	// - "ninguna circunstancia" → should be "ninguna" but here it's fine
	// - "había muerto" → should be "había muerto" (correct) but let's use "habia muerto"
	// - "nunca visitaban" → "nunca las visitaba" (missing pronoun agreement)
	// - "había enviado" → "habia enviado" (missing accent)
	// - "protestaban" → should be "protestaban" (correct) but "se quejaban" is more natural
